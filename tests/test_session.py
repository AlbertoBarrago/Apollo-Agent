...
class TestSessionManagement(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.expected_filename = f"chat_history_{self.mock_today.strftime('%Y%m%d')}.json"
        self.expected_filepath = os.path.join(self.mock_chat_history_dir, self.expected_filename)
    ...
    def test_get_daily_session_filename(self):
        ...
        with patch('datetime.date') as mock_date_module: # Renamed to avoid conflict
            ...
            expected_path = os.path.join(base_dir, f"chat_history_{self.mock_today.strftime('%Y%m%d')}.json")
            self.assertEqual(session_service.get_daily_session_filename(base_dir), expected_path)
    ...
    @patch.object(apollo_const.Constant, 'system_new_session', new_callable=unittest.mock.PropertyMock)
    ...
    def run_save_test(self,
        ...
        mock_const_system_session_prop.return_value = self.mock_system_new_session_template
        ...
        with patch('builtins.open', m_open_mock):
            returned_path = session_service.save_user_history_to_json(message_to_save, role_to_save)
        ...
        return (returned_path, mock_os_makedirs, mock_os_exists,
                mock_json_load, mock_json_dump, mock_time_strftime,
                mock_builtin_print, m_open_mock, actual_write_handle)
    ...
    def test_save_invalid_input(self):
        ...
        mock_print.assert_any_call("[WARNING] Invalid message content or role provided. Skipping save.")
    ...
    def test_save_typeerror_on_json_dump(self):
        ...
        with patch('builtins.print') as mock_p, \
             patch('time.strftime', return_value=self.mock_strftime_val), \
             patch('json.dump', side_effect=TypeError("Not serializable")) as mock_jd, \
             patch('json.load', return_value=[]), \
             patch('os.path.exists', return_value=False), \
             patch('os.makedirs'), \
             patch('apollo.service.session.get_daily_session_filename', return_value=self.expected_filepath), \
             patch.object(apollo_const.Constant, 'chat_history_dir', self.mock_chat_history_dir), \
             patch.object(apollo_const.Constant, 'max_history_messages', self.mock_max_messages), \
             patch.object(apollo_const.Constant, 'system_new_session', self.mock_system_new_session_template):

            m_open = mock_open()
            with patch('builtins.open', m_open):
                returned_path_direct = session_service.save_user_history_to_json(msg, role)

            self.assertEqual(returned_path_direct, self.expected_filepath)
            mock_p.assert_any_call(f"[ERROR] JSON serialization error: Not serializable")
            mock_p.assert_any_call("Not saving chat history due to serialization error. Please check message structure.")
