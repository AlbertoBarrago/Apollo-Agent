...
def save_user_history_to_json(message: str, role: str):
    ...
    if os.path.exists(file_path):
        try:
            ...
            print(f"[WARNING] Chat history file '{file_path}' corrupted. Starting new history for today.")
        ...
        if not is_new_session_for_today and current_history:
            ...
            if not is_system_marker_present_at_start:
                ...
                print(f"[INFO] System marker missing or malformed in '{file_path}'. Re-initializing session for today.")
                is_new_session_for_today = True
    else:
        is_new_session_for_today = True # File does not exist, so it's a new session for today
