...
This script imports ApolloAgent from both the original old_agent.py and the new apollo_agent package.
"""

import asyncio

# Test importing from the original old_agent.py
print("Testing import from old_agent.py:")
from old_agent import ApolloAgent as AgentFromOriginal
print(f"Successfully imported ApolloAgent from old_agent.py: {AgentFromOriginal}")
...
async def test_instances():
    ...
    print(f"Created instance from old_agent.py: {agent1}")
    ...
    print(f"list_dir result from old_agent.py instance: {result1['success'] if 'success' in result1 else 'OK'}")
